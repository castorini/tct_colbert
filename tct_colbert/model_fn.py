	...
	def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
		...
		if (is_training) or (is_eval):
			(total_loss, score, doc_length) = create_model(
				...
				colbert_dim, dotbert_dim, max_q_len, max_p_len, doc_type, loss, kd_source, eval_model)
		elif is_output:
		...
		if init_checkpoint:
			...
			# (assignment_map0, initialized_variable_names) = modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint, 'Teacher/')
			# (assignment_map1, initialized_variable_names1) = modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint, 'Student/', 'Student1/')
			# (assignment_map1, initialized_variable_names1
			# ) = modeling.get_assignment_map_from_checkpoint(tvars, init_checkpoint, 'Student/')
			# assignment_maps = [assignment_map, assignment_map1]
			# initialized_variable_names.update(initialized_variable_names1)

			# assignment_maps = [assignment_map0, assignment_map1]
			...
			if use_tpu:
				def tpu_scaffold():
					...
					tf.train.init_from_checkpoint(init_checkpoint, assignment_map)
					# tf.train.init_from_checkpoint(init_checkpoint, {'Siamese/':'Student/'})
					# tf.train.init_from_checkpoint(init_checkpoint, assignment_map1)
