...
flags.DEFINE_string(
	"bert_pretrained_dir",
	'gs://jackir/uncased_L-12_H-768_A-12', None)
flags.DEFINE_string(
	"output_dir",
	'gs://jackir/output', None)
flags.DEFINE_string(
	"data_dir",
	'gs://jackir/msmarco-passage', None)
flags.DEFINE_string(
	'train_file',
	'dataset_hard_train_tower.tf', None)
flags.DEFINE_string(
	'eval_file',
...
INIT_CHECKPOINT = os.path.join(BERT_PRETRAINED_DIR, FLAGS.init_checkpoint)
...
def prepare():
	...
	model_fn = model_fn_builder(
				...
				loss= LOSS, kd_source=KD_SOURCE, eval_model=EVAL_MODEL,
				is_eval=DO_EVAL,
				is_output=DO_OUTPUT)
...
def main(_):
	...
	if DO_OUTPUT:
		...
		eval_input_fn = input_fn_builder(
			dataset_path=DATA_DIR + "/" + EMBEDDING_FILE + '.tf',
			max_q_len=MAX_Query_LENGTH, max_p_len=MAX_Doc_LENGTH, doc_type=DOC_TYPE,
